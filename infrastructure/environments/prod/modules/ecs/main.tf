...
resource "aws_ecs_task_definition" "app" {
  ...
  container_definitions = jsonencode([
    {
      name      = "app"
      image     = "${aws_ecr_repository.app.repository_url}:build-1"
      essential = true
...
resource "aws_ecs_service" "app" {
  ...
  # CodePipeline manages the ECS task-definition revision.
  # Terraform manages the ECS service infrastructure.
  lifecycle {
    ignore_changes = [
      task_definition
    ]
